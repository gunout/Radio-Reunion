...
class ReunionRadioApp(Gtk.Window):
    def __init__(self):
        Gtk.Window.__init__(self, title="Radios de La Réunion")
        self.set_default_size(400, 300)  # Taille de la fenêtre

        # Définir l'icône de l'application
        self.set_icon_from_file("play1.png")  # Utiliser une icône représentant La Réunion

        # Initialiser les notifications système
        Notify.init("Radios de La Réunion")

        # Liste des stations de radio de La Réunion
        self.stations = {
            "Freedom": "http://freedomice.streamakaci.com/freedom.mp3",
            "Freedom 2": "http://freedomice.streamakaci.com/freedom2.mp3",
            "Reunion 1ere": "https://reunion.ice.infomaniak.ch/reunion-128.mp3",
            "Exo Fm": "https://exofmreunion.ice.infomaniak.ch/exofmreunion-64.aac",
            "Kreol Fm": "http://kreolfm.ice.infomaniak.ch/kreolfm-96.aac",
            "Neo Fm": "http://neofming.streamakaci.com/neofm.mp3",
            "Urban Hit Reunion": "https://listen.radioking.com/radio/400111/stream/452236",
            "Radio Pikan": "https://stream4.vestaradio.com/RADIOPIKAN?nocache=89908",
            "Antenne Reunion Radio": "https://rtlre.ice.infomaniak.ch/rtlre-64.aac",
        ...
        self.config_file = os.path.expanduser("~/.reunion_radio.conf")
        ...
        # Appliquer un style CSS aux couleurs du drapeau de La Réunion
        self.apply_reunion_css()

        # Titre de l'application
        self.title_label = Gtk.Label(label="Radios de La Réunion")
    ...
    def apply_reunion_css(self):
        """Applique un style CSS aux couleurs du drapeau de La Réunion."""
        css = """
        window {
            background-image: url('reunion_background.png');
            background-size: cover;
            background-repeat: no-repeat;
            background-position: center;
            color: #0055A4;  /* Bleu du drapeau */
            font-family: 'Courier New', monospace;
        }
        .title {
            font-size: 20px;
            font-weight: bold;
            color: #FFFFFF;  /* Blanc du drapeau */
            text-shadow: 0 0 5px #0055A4;  /* Bleu du drapeau */
            animation: glow 2s infinite alternate;
        }
        @keyframes glow {
            from { text-shadow: 0 0 5px #0055A4; }
            to { text-shadow: 0 0 20px #0055A4; }
        }
        .combo {
            background-color: rgba(255, 255, 255, 0.8);  /* Blanc du drapeau */
            color: #0055A4;  /* Bleu du drapeau */
            border: 1px solid #EF4135;  /* Rouge du drapeau */
            border-radius: 5px;
            padding: 5px;
            transition: background-color 0.3s, color 0.3s;
        }
        .combo:hover {
            background-color: rgba(239, 65, 53, 0.8);  /* Rouge du drapeau */
            color: #FFFFFF;  /* Blanc du drapeau */
        }
        .song-label {
            font-size: 14px;
        ...
        .play-button {
            background-color: #EF4135;  /* Rouge du drapeau */
            color: #FFFFFF;  /* Blanc du drapeau */
            border: none;
            border-radius: 5px;
            padding: 8px 16px;
    ...
    def update_song_info(self):
        # URL de la page de la radio (à adapter selon la radio sélectionnée)
        url = "https://example.com/radio"  # Remplacez par l'URL réelle de la radio
        try:
            ...
            if song_info:
                ...
                self.show_notification("Radios de La Réunion", f"Chanson en cours : {song_text}")
        except Exception as e:
